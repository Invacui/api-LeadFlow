...
export const signupSchema = Joi.object({
  email: Joi.string().email().required().message('Email must be a valid email address'),
  name: Joi.string().min(2).max(50).required().message('Name must be between 2 and 50 characters long'),
  password: Joi.string().min(8).max(128).required().message('Password must be between 8 and 128 characters long'),
  corporationName: Joi.string().min(2).max(100).required().message('Corporation name must be between 2 and 100 characters long'),
});
...
export const refreshSchema = Joi.object({
  refreshToken: Joi.string().required().message('Refresh token is required'),
});
